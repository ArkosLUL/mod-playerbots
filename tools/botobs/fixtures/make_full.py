#!/usr/bin/env python3
"""Build `full-v12.ndjson` and `full-v13.ndjson`: one small pull carrying every record type the recorder
can write.
...
The v13 file is the same pull plus what v13 adds: a ridden siege engine with its own `dmg` row, and
power type and power % on every snapshot row. The v12 file stays, so a pre-v13 reading keeps a test.
"""
...
MAXHP = {TANK: 50000, HEAL: 30000, RANGED: 32000, HUMAN: 34000, PET: 8000,
         BOSS: 2000000, ADD: 90000, SIEGE: 200000}

# (power type, power %) per unit in v13: rage, mana, mana, energy, focus, then no power at all. A
# vehicle's bar is energy, and the siege engine spends some of it between samples.
POWER = {TANK: (1, 20.0), HEAL: (0, 100.0), RANGED: (0, 100.0), HUMAN: (3, 100.0), PET: (2, 100.0),
         BOSS: (0, 0.0), ADD: (0, 0.0), SIEGE: (3, 100.0)}
...
SPOTS = {
    ...
    ADD: (14.0, 0.0, 0.0), SIEGE: (-30.0, 10.0, 0.0),
}


def build(version: int) -> list[dict]:
    records: list[dict] = []
    v13 = version >= 13
    units = [TANK, HEAL, RANGED, HUMAN, PET, BOSS, ADD] + ([SIEGE] if v13 else [])
    power = dict(POWER)

    def add(**fields):
        records.append(fields)

    add(e="hdr", v=version, ts=1789500000000, map=603, inst=4, diff=1, boss="fixtureboss",
        bin=1789499000000,
    ...
    if v13:
        add(t=1, e="unit", g=SIEGE, en=33060, n="Salvaged Siege Engine", lvl=80, mhp=MAXHP[SIEGE], b=0)
    ...
    health = {guid: float(MAXHP[guid]) for guid in units}
    ...
    def snapshot(when, targets=None, hazard=False):
        ...
        for guid in units:
            if health[guid] <= 0:
                continue
            spot = SPOTS[guid]
            row = [guid, spot[0], spot[1], spot[2], 0.5, pct(guid), 100.0,
...
def main() -> int:
    for version in (12, 13):
        records = build(version)
        out = pathlib.Path(__file__).resolve().parent / f"full-v{version}.ndjson"
