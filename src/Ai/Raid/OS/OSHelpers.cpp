...
std::vector<uint32> const DRAKE_ENTRIES = { NpcId::Tenebron,  NpcId::TenebronH, NpcId::Shadron,
                                            NpcId::ShadronH,  NpcId::Vesperon,  NpcId::VesperonH };

std::vector<uint32> const OFFTANK_PICKUP_ENTRIES = { NpcId::LavaBlaze,     NpcId::LavaBlazeH,
...
std::vector<uint32> const TWILIGHT_ADD_ENTRIES = { NpcId::AcolyteOfShadron,  NpcId::AcolyteOfShadronH,
...
Unit* TranquilizeTargetFor(Player* bot)
{
    ...
    // Molten Fury has no target conditions, so a wave sweeping past Sartharion hands him +100%
    // damage just as readily as it does a Lava Blaze. Blazes first, then drakes, then the boss.
    std::vector<std::vector<uint32>> const scan = {
        { NpcId::LavaBlaze, NpcId::LavaBlazeH },
        { NpcId::Tenebron, NpcId::TenebronH, NpcId::Shadron, NpcId::ShadronH, NpcId::Vesperon, NpcId::VesperonH },
        { NpcId::Sartharion, NpcId::SartharionH },
    };

    std::vector<Unit*> enraged;
    for (std::vector<uint32> const& tier : scan)
    {
        std::list<Creature*> found;
        bot->GetCreatureListWithEntryInGrid(found, tier, TRANQUILIZING_SHOT_RANGE);

        std::vector<Unit*> tierHits;
        for (Creature* creature : found)
        {
            ...
            if (HasMoltenFury(creature))
                tierHits.push_back(creature);
        }

        // Stable within a tier so every hunter builds the same list and the index below hands out
        // distinct targets instead of stacking three dispels on one blaze.
        std::sort(tierHits.begin(), tierHits.end(),
                  [](Unit const* lhs, Unit const* rhs) { return lhs->GetGUID() < rhs->GetGUID(); });
        enraged.insert(enraged.end(), tierHits.begin(), tierHits.end());
    }
