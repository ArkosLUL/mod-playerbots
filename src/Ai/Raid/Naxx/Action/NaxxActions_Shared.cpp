...
bool NaxxRedirectThreatAction::isUseful()
{
    return bot->getClass() == CLASS_HUNTER || bot->getClass() == CLASS_ROGUE;
}

bool NaxxRedirectThreatAction::Execute(Event /*event*/)
{
    ...
    if (dumpTarget && bot->HasAura(NaxxSpellIds::Misdirection) && botAI->CanCastSpell("steady shot", dumpTarget))
    {
...
Player* NaxxRedirectThreatAction::GetTankHolding(Unit* target)
{
    if (!target)
    {
        return nullptr;
...
int32 NaxxRedirectThreatAction::GetRedirecterIndex()
{
    Group* group = bot->GetGroup();
    if (!group)
    {
