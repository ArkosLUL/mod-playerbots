...
`vezax.formation`, `vezax.block`, `vezax.handler` and `vezax.interrupter` are derived, and each is
probed inside the helper that derives it so two call sites cannot disagree. The in-flight missile has
no world object until it lands, so `VezaxHazardListenerScript` writes it as a `haz` circle — the ~2s
window a bot can actually act in would otherwise be invisible.

The missile itself is covered under
...
### Vezax — Shadow Crash is a dodgeable missile
