...
- **The flakiness has a direction.** From the melee stack every point north of the raid answers
  ...
  unreachable, which is why the Searing Light spot sits south-west instead. The spot used to be
  **(862.737, 12.779, 409.832)** — on the mesh, and never once reached: bots re-issued the move for
  9 s without travelling a yard. Do not re-pick it because navprobe says it is walkable.
